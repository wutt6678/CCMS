#!/usr/bin/env bash
# Iteration-9 panel at max_new_tokens=1024 (768 still truncated 4/30
...
SMOKE_ID="smoke-2026-08-27-t1024-qwen35-9b"
FULL_ID="scale-b-2026-08-27-t1024-qwen35-9b"
...
echo "[chain] waiting for a GPU with >= 28 GiB free (1024-token cap)..."
GPU=$(wait_for_gpu 28 | tail -1)

echo "[chain] smoke: 5 families x 6 @ 1024 on cuda:${GPU}"
python -m causal_mllm.cli.replay \
  --input-dir outputs/families/scale_b_smoke \
  --max-families 5 --max-new-tokens 1024 \
...
echo "[chain] full panel: 20 families x 6 @ 1024 on cuda:${GPU}"
python -m causal_mllm.cli.replay \
  --input-dir outputs/families/scale_b_smoke \
  --max-new-tokens 1024 \
  --model-revision "$REVISION" \
