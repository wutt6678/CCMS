#!/usr/bin/env bash
# Iteration-9 panel at max_new_tokens=768 (512 still truncated 8/27
...
echo "[chain] waiting for a GPU with >= 28 GiB free (768-token cap)..."
GPU=$(wait_for_gpu 28 | tail -1)

echo "[chain] smoke: 5 families x 6 @ 768 on cuda:${GPU}"
...
echo "[chain] full panel: 20 families x 6 @ 768 on cuda:${GPU}"
python -m causal_mllm.cli.replay \
  --input-dir outputs/families/scale_b_smoke \
  --max-new-tokens 768 \
